...
trans <- read.transactions("./data/0804_machine_learning_02/00_live_class/data_mba.csv", format = "single", sep = ",", cols = c("id","item"), header = TRUE)
inspect(trans)
...
inspect(rules[1:5])
#     lhs             rhs support confidence coverage lift     count
# [1] {A, B, D, E} => {C} 0.06    1.0000000  0.06     1.562500  6   
# [2] {A, D, E}    => {C} 0.11    0.8461538  0.13     1.322115 11   
# [3] {A, B, D}    => {C} 0.16    0.8421053  0.19     1.315789 16   
# [4] {A, B, E}    => {C} 0.10    0.8333333  0.12     1.302083 10   
# [5] {A, D}       => {C} 0.25    0.8064516  0.31     1.260081 25   
