...
data = pd.read_csv("./data/0304_fundamentals_of_predictive_modelling/02_assumptions_in_mlr/Performance Index.csv")
data.head()
...
# Time:                        18:16:45   Log-Likelihood:                -85.916
...
# %% 3 - add fitted and residuals
data = data.assign(pred = pd.Series(model.fittedvalues))
data = data.assign(resi = pd.Series(model.resid))


# %% 4 - construct a scatter plot of predicted values versus residuals
data.plot.scatter(x = 'pred', y = 'resi')


# %% 5 - construct a q-q plot of residuals
