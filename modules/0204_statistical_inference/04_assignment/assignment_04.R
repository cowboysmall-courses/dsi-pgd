...
ess_data <- read.csv("../../../data/si/assignment/EMPLOYEE\ SATISFACTION\ SURVEY.csv", header = TRUE)
...
aggregate(satlevel ~ dept, data = ess_data, FUN = median)


kruskal.test(satlevel ~ dept, data = ess_data)

# Kruskal-Wallis rank sum test
# 
# data:  satlevel by dept
# Kruskal-Wallis chi-squared = 25.513, df = 2, p-value = 2.883e-06
...
nrow(subset(ess_data_I, satlevel > 3))
# 4
nrow(subset(ess_data_S, satlevel > 3))
# 2
nrow(subset(ess_data_F, satlevel > 3))
# 16
...
ess_data_sub <- subset(ess_data, satlevel > 3)
table(ess_data_sub$dept)
